...
// Legacy field generators for backward compatibility
function generateLegacyTargetProblems(data: BusinessProfileData): string {
  if (data.challenges && Array.isArray(data.challenges)) {
    return `Main challenges: ${data.challenges.join(', ')}`;
  }
  return 'Business challenges to be identified during consultation';
}

function generateLegacyValueProposition(data: BusinessProfileData): string {
  return data.value_proposition || 'Value proposition to be refined during campaign setup';
}

function generateLegacyMessageExample(data: BusinessProfileData): string {
  if (data.communication_style) {
    return `Sample message using ${data.communication_style} communication style - to be generated by AI`;
  }
  return 'Message examples to be generated by AI based on communication preferences';
}

function generateLegacySuccessOutcome(data: BusinessProfileData): string {
  if (data.monthly_lead_goal) {
    return `Target: ${data.monthly_lead_goal} qualified leads per month`;
  }
  return 'Success metrics to be defined during campaign planning';
}

function generateLegacyCallToAction(data: BusinessProfileData): string {
  return 'Call-to-action strategy to be developed during campaign creation';
}
