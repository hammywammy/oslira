...
// Collect form data based on the complete 9-step onboarding form
// Collect form data based on the complete 9-step onboarding form
const formData = {
    // Core business info
    ...
    // Target audience (keeping existing column name)
    target_audience: getFieldValue('target-description'),
    target_company_sizes: getFieldValue('target-size'), // Array of company sizes they target
    
    // Budget & goals
    budget: getFieldValue('budget'),
    monthly_lead_goal: parseInt(getFieldValue('monthly-lead-goal')) || null,
    
    // Pain points & preferences
    challenges: getFieldValue('challenges'), // Array of challenges
    communication_style: getFieldValue('communication-tone'),
    
    // Integration & team preferences
    integrations: getFieldValue('integrations'), // Array of desired integrations
    ...
    // Legacy fields - set to reasonable defaults since new form doesn't collect them
    target_problems: `Challenges: ${Array.isArray(getFieldValue('challenges')) ? getFieldValue('challenges').join(', ') : 'Not specified'}`,
    value_proposition: 'To be defined during campaign setup',
    message_example: 'To be generated by AI based on communication style',
    success_outcome: `Target: ${getFieldValue('monthly-lead-goal') || 'TBD'} leads per month`,
    call_to_action: 'To be defined during campaign creation'
};     
            // Validate all fields
